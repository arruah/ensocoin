...
Run with the -testnet option to run with "play ensocoins" on the test network, if you
...
Ensocoin is a multithreaded application, and deadlocks or other multithreading bugs
...
Ensocoin can be compiled with the libtcmalloc allocation library and
...
- BitcoinMiner : Generates ensocoins (if wallet is enabled).
...
However, in open source software such as Ensocoin, where everyone uses
...
pay attention to for reviewers of Ensocoin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Ensocoin uses tinyformat, which is type safe. Leave them out to avoid confusion
